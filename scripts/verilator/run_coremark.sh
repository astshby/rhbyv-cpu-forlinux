...
build_image() {
    ...
    bash scripts/verilator/build_benchmark_image.sh "${xlen}" "${output_dir}" \
        ...
        "${mode_define}" \
        "${coremark_sources[@]}" \
        >"logs/coremark-${mode}-build-rv${xlen}.log" 2>&1
}
...
calibration_dir="build/benchmark/coremark-rv${xlen}/calibration"
calibration_log="logs/coremark-calibration-rv${xlen}.log"
...
for mode in performance validation; do
    image_dir="build/benchmark/coremark-rv${xlen}/${mode}"
    run_log="logs/coremark-${mode}-rv${xlen}.log"
...
performance_log="logs/coremark-performance-rv${xlen}.log"
...
echo "PASS CoreMark RV${xlen}: iterations=${iterations} cycles=${performance_ticks} CoreMark/MHz=${score_per_mhz}"
